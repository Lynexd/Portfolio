    ...
    <div className='container mx-auto bg-zinc-900/30 w-11/12 sm:w-9/12 md:w-7/12 rounded-md flex flex-col-reverse my-16 py-10 px-8 justify-between md:flex-row md:items-center'>
      <div className='md:w-8/12'>
        <div className='space-y-2'>
          <h1 className='text-2xl font-semibold'> Merhaba,
            <span className='text-2xl font-semibold text-violet-500 hover:opacity-75 cursor-pointer'> @{githubUsername}</span>
          </h1>
          <ul className='flex flex-row font-medium gap-x-1'>
            <li key={1}>Bot/Web Developer ・</li>
            <li key={2}> 31/07/2023 ♡ </li>
          </ul>
        </div>
                ...
                <div className='truncate hover:text-clip'>
                  <div>
                    <div className='text-zinc-300 break-words mx-2 select-none leading-tight'>
                      <p><b>{discord.data.spotify.artist.includes(';') ? discord.data.spotify.artist.split(';')[0] : discord.data.spotify.artist}</b> - <b>{discord.data.spotify.song}</b> Dinliyor!</p>
                    </div>
                  </div>
                </div>
              </>
      ...
      <div className='avatar'>
        {discord ? (
          <div className={'w-36 rounded-full mb-4 md:mb-0 ring ' + (discord.data['discord_status'] == 'dnd' ? 'ring-[#ed4245]' : discord.data['discord_status'] == 'idle' ? 'ring-[#fee75c]' : discord.data['discord_status'] == 'online' ? 'ring-[#57f287]' : 'ring-[#2c2f33]')}>
            <Image className='rounded-full hover:opacity-75 transition duration-700' src={`https://cdn.discordapp.com/attachments/1411657666626850836/1411798587586777169/52c0871c28862368e5ba0885aaea1a99.jpg?ex=68b5f765&is=68b4a5e5&hm=c4e416d5b99b978d732e9a3dbea2a79b1ff18847290fdd6293de15c32155f530&`} alt='avatar' width={1024} height={1024} />
            <div className="absolute bottom-1 bg-[#0d0d10] rounded-full right-1 w-6 h-6 sm:hidden md:flex lg:flex items-center justify-center">
          ...
          <div className={'w-36 rounded-full mb-4 md:mb-0 ring ring-primary ring-offset-base-100 ring-offset-2'}>
            <Image className='rounded-full hover:opacity-75 transition duration-700' src={'https://media.tenor.com/HoocNDDrUKgAAAAj/rexx.gif'} alt='avatar' width={1024} height={1024} />
          </div>
        )}
      </div>
    </div>
